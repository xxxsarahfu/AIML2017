...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x,y) < 2 and (x,y) not in region:
                contiguous_region += 1
                tmp = [(x,y)]
                while tmp:
                    digit = tmp.pop()
                    region.add(digit)
                    for neighbor in getNeighbors(*digit):
                        if datum.getPixel(*neighbor) < 2 and neighbor not in region:
                            tmp.append(neighbor)   
    features["contiguous0"] = contiguous_region % 2
    features["contiguous1"] = (contiguous_region >> 1) % 2
    features["contiguous2"] = (contiguous_region >> 2) % 2
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    #util.raiseNotDefined()
    #design a good heuristic such as position of foods, capsules and ghosts.
