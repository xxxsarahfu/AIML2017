...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance) #P(D|h)
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        allPossible = util.Counter()
        ...
        if noisyDistance == None:
            self.particles = [self.getJailPosition] * self.numParticles # *weight 
        else:
            beliefDistribution = self.getBeliefDistribution()
            for pos in self.legalPositions:
                TrueDistance = util.manhattanDistance(pos,pacmanPosition)
                if emissionModel[TrueDistance] > 0:
                    allPossible[pos] = emissionModel[TrueDistance] * beliefDistribution[pos]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        for counter, particle in enumerate(self.particles): # particle = self.particles[counter]
            intermediate = 1.0 #intermediate result
            for i in range(self.numGhosts): # i = ghostIndex
                if noisyDistances[i] == None:    
                    particle = self.getParticleWithGhostInJail(particle,i)
                else:
                    trueDistance = util.manhattanDistance(particle[i],pacmanPosition)
                    intermediate *= emissionModels[i][trueDistance]
            allPossible[particle] += intermediate
